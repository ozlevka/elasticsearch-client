...
ElasticClient.prototype.indexObject = function(object, id, parent, callback) {
	...
	var requestObject =  {
		url : this.url + '/' + this.index + '/' + this.type + (id ? '/' + id : '') + (parent? '?parent=' + parent : ''),
		method : 'POST',
		json : object
	};
...
/*ElasticClient.prototy.getDateAggregation = function(from, to, step, units, callback) {
...
ElasticClient.prototype.runQuery = function(query, options, callback) {
    ...
    var index = options.index? options.index : this.index;
    var type = options.type? options.type : this.type;

    var url = this.url;
    if(!options.avoid_index) {
        url += '/' + index;
    } else if(!options.avoid_type) {
        url += '/' + type;
    }

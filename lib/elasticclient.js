...
ElasticClient.prototype.indexBulk = function(objects, options, callback) {
    ...
    var requestData = '';
    var indexTemplate = '';
    indexTemplate = '{ "index" : { "_index" : "' + this.index + '", "_type" : "' + this.type + '" %s} }';
    ...
    for(var o in objects) {
        if(options) {
            ...
            var addStr = '';
            for(k in keys) {
                addStr += util.format(addTemplate, k, options[k]);
            }
            indexTemplate = util.format(indexTemplate, addStr);
        }
        else
            indexTemplate = util.format(indexTemplate, '');
